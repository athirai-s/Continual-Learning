"""
train_carc_synthetic.py — CARC training script for the synthetic dataset.

Usage:
    python train_carc_synthetic.py --method full_ft
    python train_carc_synthetic.py --method lora
    python train_carc_synthetic.py --method smf
    python train_carc_synthetic.py --method casm

Trains on the 4 synthetic periods (2018, 2020, 2022, 2024) using
meta-llama/Llama-3.2-3B-Instruct by default. Checkpoints are saved to
/scratch1/ramyakri/checkpoints/<run_id>/.

Dataset: data/augmented/synthetic/<period>.csv + data/probes.json
(605 facts, ~150 changed probes per period boundary).
"""
...
# CLI arguments
# ---------------------------------------------------------------------------
parser = argparse.ArgumentParser(description="Continual-Learning CARC trainer (synthetic)")
parser.add_argument(
    "--method",
    required=True,
...
parser.add_argument("--periods", nargs="+", default=["2018", "2020", "2022", "2024"], help="Periods to train")
...
if args.run_id is not None:
    ...
    RUN_ID = f"step2_synth_{METHOD}_{model_tag}"

# Default learning rates per method
DEFAULT_LR = {
    "full_ft": 2e-5,
    "lora": 2e-4,
    "smf": 1e-3,
...
# ---- CASM settings ----
# Period-deterministic slot assignment: num_slots == len(PERIODS) * slots_per_period
CASM_SLOTS_PER_PERIOD = 8
CASM_NUM_SLOTS = CASM_SLOTS_PER_PERIOD * len(PERIODS)  # 8 * 4 = 32
...
# Must stay False — branching adds slots dynamically, incompatible with period-deterministic map
...
# ---------------------------------------------------------------------------
print("=" * 60)
print("CARC TRAINING — Synthetic Dataset (Continual Learning)")
print("=" * 60)
...
probes_path = os.path.join(REPO_DIR, "data", "probes.json")
assert os.path.exists(probes_path), (
    f"probes.json not found at {probes_path}. Run data/build_probes.py first."
)
print(f"probes.json: OK ({os.path.getsize(probes_path):,} bytes)")
...
print("\nAll synthetic dataset files present — ready to train.\n")
...
# ---------------------------------------------------------------------------
print("\n" + "=" * 50)
print(f"SAMPLE GENERATIONS ({PERIODS[-1]} changed probes)")
print("=" * 50)
...
# For CASM with period-deterministic routing, set the current period before generation
if hasattr(model, "_current_period"):
    model._current_period = PERIODS[-1]
